...
class JudgeProfile extends Component {
    ...
    componentDidMount() {
        let decodeddata = decode(localStorage.getItem('judgeToken')); 
        this.setState({firstname:decodeddata.judge.firstname})
        this.setState({lastname:decodeddata.judge.lastname})
        this.setState({email:decodeddata.judge.email})
        this.setState({designation:decodeddata.judge.designation})
        this.props.onchangeLocation(this.state.firstname + " " + this.state.lastname);
        console.log(decodeddata.judge.designation)
        axios.get('/api/judges/'+decodeddata.judge.id)
            .then(res => {
    ...
    handleEdit = event =>{
        this.props.history.push('/admin/profile/editprofile')
    }
    render() {
        return (
            <div>
...
export default withRouter(JudgeProfile)